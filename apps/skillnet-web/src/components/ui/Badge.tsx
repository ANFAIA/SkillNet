type BadgeVariant = 'primary' | 'accent' | 'warning' | 'danger'
type BadgeStyle = 'default' | 'plain'
...
export function Badge({
  ...
  if (badgeStyle === 'plain') {
    return (
      <span
        className={`inline-flex items-center text-xs font-semibold ${variantTextClasses[variant]} ${className}`}
      >
        {children}
      </span>
    )

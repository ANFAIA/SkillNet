...
import { LearningPreferencePreview } from './LearningPreferencePreview'
...
/**
 * The learner personalization form — web presentation, companion modalities,
 * interaction, detail, images and accessibility — plus its own autosave status.
 *
 * Extracted from the employee `LearningPreferences` page so the individual-mode
 * owner (an admin who also learns) can edit the very same preferences inside
 * admin Settings, with one implementation and no duplication. `showAppearance`
 * controls whether the theme block rides on top: the employee page has no other
 * appearance control so it shows it, while admin Settings already renders
 * `AppearanceSettings` and passes `false` to avoid showing it twice.
 */
export function LearningPreferencesSection({
  ...
  return (
    <>
      {!loading && !loadError && (pending || saved) && (
      ...
      <Card className="!p-0 overflow-hidden">
        {showAppearance && <AppearanceSettings embedded compact className="p-4 sm:p-5" />}

        <div className={showAppearance ? 'border-t border-border' : ''}>
          {loading ? (

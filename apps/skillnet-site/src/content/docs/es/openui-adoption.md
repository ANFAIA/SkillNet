...
- Los mensajes de error del parser dejan de adivinar. Cada uno nombra una causa **comprobada** y dice
  qué escribir en su lugar: el argumento con nombre (`gap = "md"`) se llama argumento con nombre, y
  el diagnóstico de la comilla sin escapar sólo aparece cuando de verdad acaba de cerrarse un texto.
  Son parte del contrato con el LLM, porque el bucle de reparación los reenvía literalmente.
  **Corrección del 2026-07-27:** la "declaración partida en dos líneas" ya no es un error. `lang-core`
  separa sentencias por saltos de línea **a profundidad de corchete 0** y nosotros separábamos por
  todos, que es exactamente el fallo de §"subconjunto estricto" otra vez. `src/render/lines.py` une
  las líneas físicas mientras haya un corchete abierto; un texto sigue teniendo que cerrar su comilla
  en su propia línea.
...
- `UI_REPAIR_SYSTEM` gana tres pares MAL/BIEN, porque son fallos que el modelo cometió pese a que el
  prompt ya los prohibía en prosa. `PROMPT_VERSION` pasa a `runtime/2`, así que invalida la caché de
  renders.
  **Corrección del 2026-07-27:** el par de la "llamada partida en varias líneas" se retiró — enseñaba
  una regla que no existe y gastaba el único reintento en reformatear un programa válido. En su sitio
  hay un par de identificador con tilde y otro de clave de respuestas. `PROMPT_VERSION` es `runtime/3`.

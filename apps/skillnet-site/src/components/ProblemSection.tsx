...
export default function ProblemSection({ lang = "es" }: { lang?: Locale }) {
  ...
  return <section id="el-problema" data-nav-theme="dark" ref={sectionRef} className="relative w-full overflow-hidden bg-[var(--color-primary-deep)]">
    ...
    <div className="relative mx-auto grid w-full max-w-[80%] gap-10 py-24 sm:min-h-[70vh] sm:grid-cols-2 sm:items-center sm:gap-16 sm:py-32">
      <motion.blockquote {...reveal} transition={{ duration: 0.6, ease: [0.38, 0.49, 0, 1] }} className="type-section-title text-balance text-white">&ldquo;{copy.quote}&rdquo;</motion.blockquote>
      <motion.p {...reveal} transition={{ duration: 0.6, delay: reduced ? 0 : 0.12, ease: [0.38, 0.49, 0, 1] }} className="type-body text-white/85">{copy.explanation}</motion.p>
    </div>
  </section>;
}

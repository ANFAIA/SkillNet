...
export default function WhatWeDoSection({ lang = "es" }: { lang?: Locale }) {
  ...
  return <section id="que-es-skillnet" data-nav-theme="light" className="relative w-full scroll-mt-24 bg-white px-6 pb-10 pt-24 sm:px-10 sm:pb-12 sm:pt-28">
    ...
    <div className="relative mx-auto w-full max-w-[80%]">
      <motion.h2 {...reveal} transition={{ duration: 0.65, ease: [0.38, 0.49, 0, 1] }} className="type-section-title text-[var(--color-text)]">{copy.title}</motion.h2>
      <motion.p {...reveal} transition={{ duration: 0.65, delay: reduced ? 0 : 0.1, ease: [0.38, 0.49, 0, 1] }} className="type-lead mt-7 w-full text-[var(--color-text-secondary)]">{copy.body}</motion.p>
    </div>
  </section>;
}

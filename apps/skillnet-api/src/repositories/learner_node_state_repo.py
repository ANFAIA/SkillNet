...
class LearnerNodeStateRepository(BaseRepository[LearnerNodeState]):
    ...
    async def mark_completed(
        ...
        **``state`` and ``mastery`` are deliberately untouched.** Finishing the reading
        ...
        figure on the same scale as measured ones — the scale ``SkillService.record_mastery``
        derives a skill level from. The two facts stay in two columns.
    ...
    async def waive(
        ...
        *,
        ...
        read by ``SkillService.record_mastery`` as evidence nobody produced. Closing the
        course is what the waiver is for; putting a level on it is not.
        """
        moment = now or datetime.now(timezone.utc)

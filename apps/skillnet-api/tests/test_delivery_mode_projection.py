"""``delivery_mode`` on the v1 read schemas: promised by §11.3, and flag-safe.
...
The property that makes it safe to add to a v1 surface is the one asserted here: the value
is **``resolve_delivery``**, not ``courses.delivery_mode``. So

* with the flag ``off`` every course reads ``static`` — the flag-off deployment is
  byte-identical to what v1 served, which is the condition §10.1 puts on every v2 field;
* a course opted into ``dynamic`` whose schema is still in draft also reads ``static``,
  because that is the path it is actually served on. A badge saying "dinamico" over it
  would send the creator hunting for a node map that does not exist.
...
already owns the full 3x2x2 truth table of the decision itself.
"""
...
#: (flag, course delivery_mode, schema_status) -> what every read schema must print.
CASES = (
    ("on", CourseDeliveryMode.DYNAMIC, CourseSchemaStatus.VALIDATED, "dynamic"),
    ("on", CourseDeliveryMode.DYNAMIC, CourseSchemaStatus.DRAFT, "static"),
    ("on", CourseDeliveryMode.STATIC, CourseSchemaStatus.VALIDATED, "static"),
    ("shadow", CourseDeliveryMode.DYNAMIC, CourseSchemaStatus.VALIDATED, "static"),
    ("off", CourseDeliveryMode.DYNAMIC, CourseSchemaStatus.VALIDATED, "static"),
)


@pytest.mark.parametrize(("mode", "delivery", "schema_status", "expected"), CASES)
...
def test_an_enrollment_with_no_course_loaded_reads_static(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The same value the flag-off deployment produces: absence of information is never
    a reason to advertise a surface."""

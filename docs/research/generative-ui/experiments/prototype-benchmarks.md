...
### P3: Declarative (UIDL Level 2)

Compact UIDL specs processed by an existing renderer into standalone HTML. No LLM in the rendering process.

| Spec | UIDL chars | UIDL tokens | HTML chars |
|------|-----------|-------------|------------|
...
**Compression ratio:** UIDL is 5.7x more compact than the resulting HTML.
...
| | P1 Static | P2 Generative | P3 UIDL | P4 Bidirectional | P5 Vault-to-Page |
|---|---------|-----------|---------|------------------|---------------|
| **Level** | 1 | 3 | 2 | 3+ (collaborative) | 2 |
| **Tokens/page** | ~300/interaction | 3,343/page | 0 (or ~458 if agent generates UIDL) | ~3,500/cycle | 0 |
...
*UIDL can be generated by a human or an agent. The renderer is deterministic.

---

## Key Findings

### 1. Level 2 (UIDL) is 7.3x more token-efficient than Level 3
...
- **Level 2 UIDL:** 458 tokens average per spec (LLM only generates the description)
- **Ratio: 7.3x** fewer tokens with UIDL
...
P4 demonstrates the concept is viable: agent generates UI, user interacts, agent re-generates. But each cycle costs ~3.5K tokens and ~25s. For a product, this requires: caching previous cycles, using Level 2 (UIDL) instead of Level 3 to reduce tokens, and pre-generating probable options.
...
The three HTML files generated by the LLM are functional but each has different styles. Level 2 (UIDL) produces consistent output because it uses the same design system (renderer.js). This matters for a product.
...
| Dashboards and reports | P3: UIDL generated by agent | ~458 | <1s |

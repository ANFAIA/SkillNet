...
- Setting `POSTGRES_PASSWORD` to a strong random value. `.env.example` ships the line empty, so `docker compose up` refuses to start until it is filled in — and it must use letters, digits, `-` and `_` only, because it goes into the connection URL unescaped. See [`configuration.md`](configuration.md).

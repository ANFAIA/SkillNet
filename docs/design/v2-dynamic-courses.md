...
>
> Requisito duro: **el camino estático de v1 debe seguir funcionando** con el flag apagado.
...
### 10.1 El flag principal

**Env var: `DYNAMIC_COURSES_MODE`.** Añadida a `src/config.py::Settings` como
`DYNAMIC_COURSES_MODE: Literal["off", "shadow", "on"] = "off"`.

**Se expone SÓLO en `GET /api/v1/health`**, que ya devuelve un dict plano
(`src/routes/health.py:14-22`), y el cliente lo lee **una vez al arrancar**:

```json
{"status": "ok", "version": "0.1.0", "database": "connected",
 "features": {"dynamic_courses": "off"}}
```

**No se expone en `GET /auth/me`**, y esto no es preferencia estética. Esa ruta hace
`return user` sobre el objeto ORM (`src/routes/auth.py:17-19`) y deja que
`UserRead` (`from_attributes=True`, `src/auth/schemas.py:9-17`) lo serialice; `src/models/user.py` no
tiene columna `features`. Añadir el campo al schema daría uno de dos resultados, ambos malos: un
`ValidationError` en cada llamada a `/auth/me`, o —con un default— **el default serializado siempre**,
es decir el frontend leyendo `"off"` para toda la eternidad y desactivando en silencio todas las
pantallas v2 con el flag encendido. Si en el futuro hace falta ahí, la forma correcta es construir la
respuesta explícitamente
(`UserRead.model_validate(user).model_copy(update={"features": …})`), no tocar el schema.

| Valor | Backend | Frontend | Para qué |
|-------|---------|----------|----------|
| **`off`** (default) | Todas las rutas de §11 devuelven `404`. Los grafos nuevos no se registran. `courses.delivery_mode` se ignora. `POST /courses/{id}/generate` v1 intacto | No se monta `/onboarding` ni `NodeView`. `CourseView` como hoy | **Producción tal cual está.** Estado por defecto tras el merge. Las migraciones se aplican pero ninguna columna nueva se usa |
| **`shadow`** | Rutas de **admin** activas (`schema/propose`, `schema`, `schema/validate`, y `render` con `?preview=1`). Rutas de **empleado** (`probe`, `answer`, `explain`, onboarding) devuelven `404` | El admin ve la pestaña "Esquema" y puede previsualizar renders con su propio perfil. El empleado ve exactamente lo de v1 | Validar la calidad del esquema y de los renders **con datos reales sin exponer a nadie**. Es el modo de la demo interna |
| **`on`** | Todo activo. Un curso va por v2 sólo si `delivery_mode='dynamic'` **y** `schema_status='validated'`. Cualquier otro curso sigue por v1 en la misma instancia | Gate de onboarding activo. `CourseView` decide: si el curso es `dynamic` → lista de nodos + `NodeView`; si no → árbol de módulos de v1 | Producción v2 |

La condición completa que decide el camino, en un único sitio
(`src/services/course_delivery.py::resolve_delivery(course, settings)`):

```python
def resolve_delivery(course, settings) -> Literal["static", "dynamic"]:
...
Un único punto de decisión, testeable con una tabla de verdad de 12 casos. Prohibido consultar
`settings.DYNAMIC_COURSES_MODE` en cualquier otro sitio que no sean los guards de ruta y esta
función.

### 10.2 Flags secundarias
...
`docker-compose.yml` gana un perfil `fixtures` con `LLM_MODEL=fixture/local`,
`EMBEDDING_MODEL=fixture/local` y `DYNAMIC_COURSES_MODE=on`, para que el flujo completo se pueda
demostrar en local sin ninguna clave. Funciona en el compose de producción porque las fixtures viajan
...
| Integración | `tests/integration/test_v1_regression.py` (`@pytest.mark.integration`) | Con `DYNAMIC_COURSES_MODE=off`: el flujo v1 completo sigue idéntico y las rutas nuevas dan 404 | docker db |

...
CREATE TABLE api_keys (
    ...
    key_prefix  text NOT NULL,              -- "sk_live_a3f2" (primeros 8 chars, para identificar)
...
**Formato del key:** `sk_live_{32_chars_random}` (ejemplo: `sk_live_EXAMPLE_KEY_REPLACE_ME_1234567890`). El prefijo `sk_live_` permite reconocer que es un key de SkillNet. El key completo se muestra al usuario UNA sola vez al crearlo. En la base de datos se guarda solo el hash.

**Envio en request:**

```http
GET /ext/v1/skills
Authorization: Bearer sk_live_EXAMPLE_KEY_REPLACE_ME_1234567890
```
